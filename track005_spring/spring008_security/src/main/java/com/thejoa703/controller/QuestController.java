...
public class QuestController { 
	...
	*/
	...
	// 글쓰기 폼
	//@PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_MEMBER')") // 안에있는 권한중 아무거만 있어도 글쓰기 가능
	//@PreAuthorize("isAuthenticated() and hasRole('ROLE_ADMIN')") // 로그인 + ADMIN권한 있다면 글쓰기 가능
	//@PreAuthorize("isAnonymous()") // 아무나 글쓰기 가능
	...
	@RequestMapping(value="/updateEdit.quest" 
			, method=RequestMethod.POST //수정기능
			, headers=("content-type=multipart/*"))
	public String edit_post(
			@RequestParam("file") MultipartFile file, Sboard1Dto dto, RedirectAttributes rttr) { 
