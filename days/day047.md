...
[디자인패턴 + 라이브러리 = 프레임워크]
1) 디자인패턴(가이드라인) 
2) 라이브러리 (특정기술 구현을 라이브러리의 형태)
	$("#id").on("click",); , $.ajax()

- 소프트웨어 개발의 뼈대 역할
- 애플리케이션 최소한의 공통점을 찾아 하부구조 제공
...
- POJO(Plain Old Java Object)
 ...
 ※ IOC가 아닌경우
 	[개발자가 생성 → Class BList
 		 개발자가 생성 → Class BInsert
		 	개발자가 생성 → Class BEdit] 컨테이너

 ※ IOC인 경우
	[개발자가 사용 → Class BList
 		 개발자가 사용 → Class BInsert
		 	개발자가 사용 → Class BEdit] 컨테이너
