...
1) 정보보관 : UserInfo [ -id, -pass, -balabce ]
2) 기능클래스 :
   Add     [exec()]
   Show    [exec()]
   Deposit [exec()]
...
5. uml
1) 연관관계(연결 → , 한쪽은 알지만, 다른쪽은 상대방의 존재 모름)
2) 일반화관계(상속, IS-A 고양이는 동물이다, 속이 빈 화살표)
     고양이는 동물이다(o), 사람도 동물이다(o), 동물은 고양이다(x)
3) 집합관계(자동차 = 엔진 + 휠
     ◆ 생성자 안에서 new(합성), ◇ 생성자 주소받아서 사용(집약))
4) 의존관계
5) 실체화관계(인터페이스)

6. SOLID 원칙
S : 단일책임       - 한 클래스는 한가지 일만 한다(쿠기굽기) 
o : 개방폐쇄       - 새로운 쿠키는 쉽게 추가 가능(확장O, 수정X)
L : 리스코프 치환   - 모든쿠키는 같은방식으로 만들 수 있다
I : 인터페이스 분리 - 필요한 기능만 
