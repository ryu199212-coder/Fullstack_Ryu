...
async function findUserByEmail(email){
    ...
    try{
        ...
        const result = await conn.execute( 
            `SELECT APP_USER_ID, EMAIL,PASSWORD, NICKNAME, MOBILE, MBTI_TYPE_ID, CREATED_AT 
            FROM APPUSER1  WHERE EMAIL= :email` , 
            {email} , 
            options  
        ); //실행
        //결과처리
    ...
    try{
        ...
        const result = await conn.execute( 
            `SELECT APP_USER_ID, EMAIL, PASSWORD, NICKNAME, MOBILE, MBTI_TYPE_ID, UFILE, CREATED_AT
             FROM APPUSER1 WHERE APP_USER_ID = :id` , 
             {id} , 
             options  ); //실행
        //결과처리
        return result.rows[0];
...
// 전체사용자조회(테스트용)
// SELECT APP_USER_ID, EMAIL, NICKNAME, MOBILE, MBTI_TYPE_ID, CREATED_AT FROM APPUSER1
...
// 사용자 닉네임 수정 (Update)
// UPDATE APPUSER1 SET NICKNAME= :nickname WHERE APP_USER_ID= :id
...
// 사용자 삭제 (Delete)
// DELETE FROM APPUSER1 WHERE APP_USER_ID = :id
